...
def get_azure_credentials(config, args):
    """Get Azure credentials based on config, args, and env vars"""
    auth_method = args.auth_method or os.environ.get('AZURE_AUTH_METHOD') or config.get('azure', {}).get('authentication', {}).get('method', 'default')
    
    client_id = args.azure_client_id or os.environ.get('AZURE_CLIENT_ID') or config.get('azure', {}).get('authentication', {}).get('client_id')
    client_secret = args.azure_client_secret or os.environ.get('AZURE_CLIENT_SECRET') or config.get('azure', {}).get('authentication', {}).get('client_secret')
    tenant_id = args.azure_tenant_id or os.environ.get('AZURE_TENANT_ID') or config.get('azure', {}).get('authentication', {}).get('tenant_id')
...
def get_or_create_tag(nb, tag_input, tag_description="", tag_color="aaaaaa"):
    if isinstance(tag_input, dict):
        ...
        if not tag_name:
            logger.error("Tag dict missing 'name' key; skipping.")
            return None
        tag_description = tag_input.get('description', tag_description)
        tag_color = tag_input.get('color', tag_color)
    elif isinstance(tag_input, str):
        tag_name = tag_input
    else:
        logger.error(f"Invalid tag input: {tag_input}; skipping.")
        return None
    ...
    try:
        ...
        if tag:
            ...
            if needs_update:
                tag.save()
            logger.info(f"Found/Updated tag: {tag_name} (color: {tag_color})")
            return tag
    except Exception as e:
...
def get_or_create_ipam_role(nb, name, slug, description, tags):
    ...
    tag_dicts = [{'id': get_or_create_tag(nb, t).id} for t in tags if get_or_create_tag(nb, t)]
...
def get_or_create_tenant(nb, name, group=None, description="", slug=None, tags=None):
    ...
    try:
        ...
        if tenant:
            needs_update = False
            current_group_id = tenant.group.id if tenant.group else None
            new_group_id = group.id if group else None
            if current_group_id != new_group_id:
                tenant.group = new_group_id
                needs_update = True
...
def get_or_create_prefix(nb, prefix_value, defaults, subscription_name=None, subscription_id=None, aggregate_id=None, role_id=None, tenant_id=None, tags=None):
    try:
        ...
        if existing_prefixes:
            ...
            # Reload the full prefix object to ensure all fields are populated
            prefix = nb.ipam.prefixes.get(prefix.id)
            logger.debug(f"Reloaded full prefix: {prefix_value}")
            
            changes = {}
            
            # Handle description
            if 'description' in defaults:
                new_desc = defaults['description']
                current_desc = getattr(prefix, 'description', '')
                logger.debug(f"Checking description for {prefix_value}: current={current_desc!r}, new={new_desc!r}")
                if current_desc != new_desc:
                    changes['description'] = new_desc
                    logger.debug(f"Queued update for description from {current_desc!r} to {new_desc!r}")
            
            # Handle status
            if 'status' in defaults:
                new_status = defaults['status']
                current_status = getattr(prefix, 'status', None)
                current_status_value = None
                if isinstance(current_status, dict):
                    current_status_value = current_status.get('value')
                elif hasattr(current_status, 'value'):
                    current_status_value = current_status.value
                else:
                    current_status_value = current_status
                logger.debug(f"Checking status for {prefix_value}: current={current_status_value!r}, new={new_status!r}")
                if current_status_value != new_status:
                    changes['status'] = new_status
                    logger.debug(f"Queued update for status from {current_status_value!r} to {new_status!r}")
            
            # Handle aggregate
            current_aggregate_id = None
            if hasattr(prefix, 'aggregate') and prefix.aggregate is not None:
                if isinstance(prefix.aggregate, dict):
                    current_aggregate_id = prefix.aggregate.get('id')
                else:
                    current_aggregate_id = getattr(prefix.aggregate, 'id', None)
            if aggregate_id and current_aggregate_id != aggregate_id:
                changes['aggregate'] = {'id': aggregate_id}
                logger.debug(f"Queued update for aggregate to {aggregate_id}")
            
            # Handle role
            current_role_id = None
            if hasattr(prefix, 'role') and prefix.role is not None:
                if isinstance(prefix.role, dict):
                    current_role_id = prefix.role.get('id')
                else:
                    current_role_id = getattr(prefix.role, 'id', None)
            if role_id and current_role_id != role_id:
                changes['role'] = {'id': role_id}
                logger.debug(f"Queued update for role to {role_id}")
            
            # Handle tenant
            current_tenant_id = None
            if hasattr(prefix, 'tenant') and prefix.tenant is not None:
                if isinstance(prefix.tenant, dict):
                    current_tenant_id = prefix.tenant.get('id')
                else:
                    current_tenant_id = getattr(prefix.tenant, 'id', None)
            if tenant_id and current_tenant_id != tenant_id:
                changes['tenant'] = {'id': tenant_id}
                logger.debug(f"Queued update for tenant to {tenant_id}")
            
            # Handle tags
            current_tags = set()
            if hasattr(prefix, 'tags') and prefix.tags:
                for tag in prefix.tags:
                    if isinstance(tag, dict):
                        current_tags.add(tag.get('id'))
                    else:
                        current_tags.add(getattr(tag, 'id', None))
            new_tags = set(tag['id'] for tag in tags)
            if current_tags != new_tags:
                changes['tags'] = tags
                logger.debug(f"Queued update for tags")
            
            # Handle custom fields
            custom_fields = dict(getattr(prefix, 'custom_fields', {}) or {})
            ...
            if subscription_name and subscription_id:
                ...
                if existing_value != azure_subscription_value or re.search(r'[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}', existing_value, re.IGNORECASE):
                    ...
                    logger.debug(f"Queued update for azure_subscription")
            
            if 'custom_fields' in defaults and 'peering_check' in defaults['custom_fields']:
                new_peering = defaults['custom_fields']['peering_check']
                current_peering = custom_fields.get('peering_check', None)
                logger.debug(f"Checking peering_check for {prefix_value}: current={current_peering!r}, new={new_peering!r}")
                if current_peering != new_peering:
                    custom_fields['peering_check'] = new_peering
                    custom_fields_updated = True
                    logger.debug(f"Queued update for peering_check")
            
            if custom_fields_updated:
                changes['custom_fields'] = custom_fields
            
            if changes:
                logger.debug(f"Applying changes to prefix {prefix_value}: {changes}")
                if prefix.update(changes):
                    logger.info(f"Successfully updated prefix: {prefix_value}")
                else:
                    logger.warning(f"Update for prefix {prefix_value} failed")
            else:
                logger.debug(f"No changes needed for prefix {prefix_value}")
...
def setup_tenancy(nb, config, all_network_data):
    ...
    for group in groups_config:
        ...
        group_map[group['name']] = created_group
    
    default_group_name = tenancy_config.get('default_group', 'AZURE')
    default_group = group_map.get(default_group_name)
    if not default_group:
        logger.error(f"Default Tenant Group {default_group_name} not found")
        return {}, ''
    
    tenant_tag_names = tenancy_config.get('tenant_tags', [])
    tenant_tags = [{'id': get_or_create_tag(nb, t).id} for t in tenant_tag_names if get_or_create_tag(nb, t)]
...
def setup_custom_tags(nb, config):
    ...
    for tag_config in custom_tags:
        ...
        color = tag_config.get('color', "aaaaaa")  # Default gray if not specified

        # Validate color is a valid 6-char hex
        if not re.match(r'^[0-9a-fA-F]{6}$', color):
            logger.error(f"Invalid color '{color}' for tag '{name}'; must be 6-character hexadecimal (e.g., 'ff0000'). Skipping.")
            continue

        get_or_create_tag(nb, name, description, color)
...
def sync_to_netbox(all_network_data, netbox_url, netbox_token, config, credential):  # Pass credential
    ...
    setup_custom_tags(nb, config)  # New: Setup custom tags from config.yaml
    ...
    azure_tag = get_or_create_tag(
        nb,
        tag_input=sync_tag_config.get('name', "azure-sync"),
...
def main():
    ...
    netbox_url = args.netbox_url or os.environ.get('NETBOX_URL') or config.get('netbox', {}).get('url')
    netbox_token = args.netbox_token or os.environ.get('NETBOX_TOKEN') or config.get('netbox', {}).get('token')
    
    if not netbox_url or not netbox_token:
        logger.error("Netbox URL and token must be provided either as arguments, environment variables, or in config.yaml")
        sys.exit(1)
    ...
    try:
        ...
        specific_id = args.subscription_id or os.environ.get('AZURE_SUBSCRIPTION_ID') or config.get('azure', {}).get('subscriptions', {}).get('specific_id')
        mg_id = args.management_group_id or os.environ.get('AZURE_MANAGEMENT_GROUP_ID') or config.get('azure', {}).get('subscriptions', {}).get('management_group', {}).get('id')
        mg_name = args.management_group_name or os.environ.get('AZURE_MANAGEMENT_GROUP_NAME') or config.get('azure', {}).get('subscriptions', {}).get('management_group', {}).get('name')
