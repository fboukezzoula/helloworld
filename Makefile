...
.PHONY: build
build:
	docker build -t $(IMAGE_REPO):$(VERSION)-$(SHA) --build-arg "AZURE_STORAGE_CONNECTION_STRING=$(AZURE_STORAGE_CONNECTION_STRING)" --build-arg "AZURE_STORAGE_CONTAINER_NAME=$(AZURE_STORAGE_CONTAINER_NAME)" .
