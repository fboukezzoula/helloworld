Great spot — that behavior means NetBox was replacing the entire tag list with just ip-availables-sync. We need to always send the full union of existing tag IDs + the sync tag ID. The safest way is to re-fetch each prefix by ID to get its current tag IDs, then patch with the union.

# Below is a drop‑in replacement for update_list_available_ips.py that:

- Fetches a fresh copy of each prefix (nb.ipam.prefixes.get(p.id)) to read current tag IDs.
- Preserves all existing tags and adds ip-availables-sync by ID.
- Uses only numeric tag IDs in payloads (required by NetBox 4.x).
- Keeps everything else the same: emoji summary, numeric fields, optional create-missing, thresholds, dry-run.

```python
#!/usr/bin/env python3
# NetBox updater: emoji summary + numeric CFs + always-apply tag (IDs only) while preserving all existing tags.
...
def get_existing_tag_ids(nb, prefix_obj):
    ...
    try:
        ...
        if fresh and hasattr(fresh, "tags"):
            ...
            if ids:
                return ids
            # fallback: resolve by slug/name if IDs missing
            return extract_tag_ids_from_obj(nb, fresh.tags)
    except Exception:
        pass
    # ultimate fallback: try whatever we have on the original object
...
# --- Main ---
def main():
    ap = argparse.ArgumentParser(description="Update NetBox CFs from azure-vnet-scan CSV. Preserve all tags and add 'ip-availables-sync' by ID.")
...
Why this fixes tag removal

NetBox replaces the entire tags list on update. We now:
Re-fetch the prefix to read its current tags with IDs
Send the union of current tag IDs + the ip-availables-sync ID
Therefore none of your existing tags get dropped.
Verify with dry run

You should now see your existing tag IDs preserved:
NETBOX_URL=… NETBOX_TOKEN=… python3 update_list_available_ips.py out.csv --dry-run
Look for: tag_ids=[…, <existing ids>, <sync tag id>]
Then run live:

python3 update_list_available_ips.py out.csv --create-missing
-  unexpected tag changes, send me one example of a prefix’s tag JSON (GET /api/ipam/prefixes/<id>/) 
