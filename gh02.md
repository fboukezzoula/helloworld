...
# --- Helper Functions ---
## NEW: Function to show usage information ##
usage() {
    echo -e "${YELLOW}Usage: $0 <ORG_NAME> <SCAN_MODE> [REPO_LIST]${NC}"
    echo
    echo "Arguments:"
    echo "  <ORG_NAME>     The name of the GitHub organization to scan."
    echo "  <SCAN_MODE>    The scan mode: 'org' or 'list'."
    echo "  [REPO_LIST]    Required if SCAN_MODE is 'list'. A single comma-separated string of repo names."
    echo
    echo "Examples:"
    echo "  Scan entire organization:  $0 my-org org"
    echo "  Scan a list of repos:      $0 my-org list \"repo-a,repo-b,repo-c\""
    exit 1
}
...
main() {
    ## MODIFIED: Argument parsing and validation moved to the top of main() ##
    if [[ $# -lt 2 ]]; then
        usage
    fi
    ...
    if [[ "$SCAN_MODE" != "org" && "$SCAN_MODE" != "list" ]]; then
        echo -e "${RED}Error: Invalid SCAN_MODE. Must be 'org' or 'list'.${NC}\n"
        usage
    fi

    if [[ "$SCAN_MODE" == "list" ]]; then
        if [[ $# -lt 3 ]]; then
            echo -e "${RED}Error: SCAN_MODE 'list' requires a third argument: a comma-separated list of repos.${NC}\n"
            usage
        fi
        # Convert comma-separated string to a bash array
        IFS=',' read -r -a REPOS_ARRAY <<< "$3"
    fi
...
examples :
```
./github_pr_report.sh <your-org-name> org
./github_pr_report.sh my-great-org org
./github_pr_report.sh <your-org-name> list "repo-one,repo-two,repo-three"
./github_pr_report.sh my-great-org list "api-gateway,user-service,frontend-app"
```
