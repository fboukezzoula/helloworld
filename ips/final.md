...
```
#!/usr/bin/env bash
# azure-vnet-scan.sh (safe-copy, sans jq "KATEX_INLINE_OPEN … )")
# Requirements: az cli, jq, python3, bash
# Garde-fous env:
#   AZ_TIMEOUT=30
#   SKIP_MG=1
#   ENABLE_IPV6=1
#   EXPAND_USED_WITH_RESOURCES=1
...
if [[ -n "$SUBS_INPUT" ]]; then
  ...
  for s in "${tmp[@]}"; do
    if az account show -s "$s" >/dev/null 2>&1; then
      ...
      log WARN "Subscription introuvable: $s"
    fi
  done
fi

# Subscriptions via MG (-m)
if [[ -n "$MGROUP" ]]; then
  ...
  if [[ "$mgjson" != "[]" ]]; then
    ...
    log WARN "Aucune subscription renvoyée par le MG '$MGROUP'."
  fi
fi

# Toutes les subscriptions (-a) ou fallback
if $ALL_SUBS || [[ ${#SUBS_LIST[@]} -eq 0 ]]; then
  [[ $ALL_SUBS == true ]] && log INFO "Récupération de toutes les subscriptions accessibles..."
...
# Mapping Subscription -> MG (optionnel)
declare -A SUB_TO_MG
if [[ "${SKIP_MG:-0}" -eq 1 ]]; then
  ...
  if [[ ${#mgroups_list[@]} -eq 0 ]]; then
    log WARN "Aucun MG listé; la colonne 'management group' sera 'N/A'."
  else
...
# Calcul used/avail pour un prefix à partir de "cidr,used"
...
for sitem in "${SUBS_LIST[@]}"; do
  ...
  for i in $(seq 0 $((vcount-1))); do
    ...
    # Map used par subnet = nombre d'ipConfigurations
    ...
    # Expansion optionnelle (LB/AppGW/Firewall/Bastion/VNGW/PLS)
