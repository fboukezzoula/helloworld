...
# The address the email will be sent from. Format: "Display Name <email@company.com>"
EMAIL_FROM="GitHub Reporter <no-reply@your-company.com>"

# The primary recipient(s) of the email report.
EMAIL_TO="dev-team@your-company.com"

# The recipient(s) to be CC'd. Can be left empty.
EMAIL_CC="engineering-manager@your-company.com"

# Your company's SMTP server. Often, you can leave this commented out if your
# server is already configured to relay mail. If not, specify it here.
...
# --- NEW: Function to send the HTML report via email ---
send_email_report() {
    if [[ "$SEND_EMAIL" != "true" ]]; then
        echo -e "\n${YELLOW}Email sending is disabled. Skipping.${NC}"
        return
    fi

    if ! command -v mailx &> /dev/null; then
        echo -e "\n${RED}${EMOJI_ERROR} 'mailx' command not found. Cannot send email.${NC}"
        echo -e "${YELLOW}On RHEL, please run: sudo dnf install mailx${NC}"
        return
    fi

    local html_file="$1"
    local subject="GitHub PR Health Report - $(date +'%Y-%m-%d')"
    local mail_cmd="mailx -s \"$subject\""

    # Add SMTP server setting if it's defined
    if [[ -n "$SMTP_SERVER" ]]; then
        mail_cmd+=" -S smtp=$SMTP_SERVER"
    fi

    # Add From, Content-Type, and CC headers
    mail_cmd+=" -a \"From: $EMAIL_FROM\""
    mail_cmd+=" -a \"Content-Type: text/html; charset=UTF-8\""
    if [[ -n "$EMAIL_CC" ]]; then
        mail_cmd+=" -c \"$EMAIL_CC\""
    fi

    # Add the primary recipient and the HTML file content
    mail_cmd+=" \"$EMAIL_TO\" < \"$html_file\""

    echo -e "\n${BLUE}Sending email report to '$EMAIL_TO'...${NC}"
    
    # Execute the command
    eval "$mail_cmd"

    echo -e "${GREEN}${EMOJI_SUCCESS} Email command executed successfully.${NC}"
}


# --- Core Logic Functions ---
get_days_open_html() {
    local created_at_iso="$1"; local pr_timestamp=$(date -d "$created_at_iso" +%s); local now_timestamp=$(date +%s)
    local seconds_diff=$((now_timestamp - pr_timestamp)); local days_open=$((seconds_diff / 86400))
    local emoji="🔵"; if [ "$days_open" -gt "$DAYS_THRESHOLD" ]; then emoji="🔴"; fi
...
process_review_prs() {
    ...
    local prs; prs=$(echo "$pr_list_json" | jq -r '.[] | [.number, .title, .url, .author.login, .createdAt, ([.reviewRequests[]? | .login // .name] | join(" ")) // "None"] | @tsv')
...
process_undeleted_branches() {
    local repo_full_name="$1"; local branches; branches=$(gh api "repos/$repo_full_name/branches" --paginate -q '.[].name' 2>/dev/null); if [[ -z "$branches" ]]; then return 0; fi
    local merged_prs_json; merged_prs_json=$(gh pr list -R "$repo_full_name" --state merged --limit 100 --json headRefName,number,title,url,mergedBy,mergedAt,isCrossRepository 2>/dev/null)
    local prs_to_check; prs_to_check=$(echo "$merged_prs_json" | jq -r '.[] | select(.isCrossRepository == false) | [.headRefName, .number, .title, .url, .mergedBy.login, (.mergedAt|fromdate|strflocaltime("%Y-%m-%d"))] | @tsv')
...
# --- Main Execution ---
main() {
    export GH_HOST="$GHE_HOSTNAME"; check_dependencies; repo_list=$(get_repo_list)
    if [[ -z "$repo_list" ]]; then echo -e "${RED}${EMOJI_ERROR} No repositories found. Please check configuration.${NC}"; exit 1; fi
    ...
    if [[ $TOTAL_PRS_AWAITING_REVIEW -eq 0 ]]; then echo -e "${GREEN}No open PRs awaiting review found.${NC}"; add_html_empty_state "No open PRs awaiting review found. Great job!" "$HTML_OUTPUT_FILE"; fi
    ...
    if [[ $TOTAL_UNDELETED_BRANCHES -eq 0 ]]; then echo -e "${GREEN}No PRs with undeleted branches found.${NC}"; add_html_empty_state "Excellent branch hygiene! No undeleted branches found." "$HTML_OUTPUT_FILE"; fi
    ...
    # NEW: Call the email function at the end
