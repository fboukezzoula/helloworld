...
# Your company's SMTP server and port (port 25 is default if not specified).
# THIS IS NOW REQUIRED. Example: "smtp.your-company.com" or "smtp.your-company.com:587"
...
# The primary recipient(s) of the email report, comma-separated.
EMAIL_TO="dev-team@your-company.com,another-dev@company.com"

# The recipient(s) to be CC'd, comma-separated. Can be left empty.
...
# --- NEW: Function to create the Python email script ---
...
# 1: SMTP Server (e.g., "smtp.example.com:25")
# 2: From Address (e.g., "Display Name <from@example.com>")
# 3: To Addresses (e.g., "to1@example.com,to2@example.com")
# 4: CC Addresses (e.g., "cc1@example.com,cc2@example.com")
# 5: Subject

SMTP_SERVER = sys.argv[1]
SENDER = sys.argv[2]
RECIPIENTS_TO = [addr.strip() for addr in sys.argv[3].split(',') if addr.strip()]
RECIPIENTS_CC = [addr.strip() for addr in sys.argv[4].split(',') if addr.strip()]
SUBJECT = sys.argv[5]

# Read the HTML content from standard input
...
try:
    with smtplib.SMTP(SMTP_SERVER) as server:
        # If your SMTP server requires authentication, add login here:
...
# --- NEW: Function to send email using the Python script ---
send_email_report() {
    ...
    if ! command -v python3 &> /dev/null; then
        echo -e "\n${RED}${EMOJI_ERROR} 'python3' command not found. Cannot send email.${NC}"
        return
    fi
    local html_file="$1"
    local subject="GitHub PR Health Report - $(date +'%Y-%m-%d')"
    echo -e "\n${BLUE}Sending email report to '$EMAIL_TO' using Python...${NC}"
    # The python script reads the HTML content from stdin (< "$html_file")
    python3 "$PYTHON_MAILER_SCRIPT" "$SMTP_SERVER" "$EMAIL_FROM" "$EMAIL_TO" "$EMAIL_CC" "$subject" < "$html_file"
...
# --- Main Execution ---
main() {
    export GH_HOST="$GHE_HOSTNAME"; check_dependencies;
    # NEW: Create the python mailer script
    create_python_mailer
